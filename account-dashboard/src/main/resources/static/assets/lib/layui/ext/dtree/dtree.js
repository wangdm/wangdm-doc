			...
			checkbox: function() {	// 复选框
				...
				if(flag){
					...
					if(typeof checkArr === "string"){
						...
						} else if (checkArr == "1") {	//选择
							CHOOSE_CLASS = LI_DIV_CHECKBAR_ON + " " + _this.style.chs;
						} else {	//未选择或者无值
							CHOOSE_CLASS = LI_DIV_CHECKBAR_OUT;
						}
						result += "<i class='"+ICONFONT+" "+_this.style.ifont+" "+_this.style.cbox+" "+CHOOSE_CLASS+"' data-id='"+treeId+"' root-id='"+rootId+"' data-checked='"+checkArr+"' data-initchecked='"+checkArr+"' data-type='0' d-click='"+eventName.checkNodeClick+"' data-par='."+LI_CLICK_CHECKBAR+"'></i>";
					}else{
